...
```{r}
seventies_show_label = literal(
  text_value = "That Seventies Show",
  lang = "en"
)

seventies_show_label
```
...
teodor_georgiev = fidentifier(
 label = "Teodor Georgiev",
 prefixes = prefixes,
 def_prefix = c(openbiodiv = "http://openbiodiv.net/"),
 simple_lookup)

lyubomir_penev = fidentifier(
 label = "Lyubomir Penev",
 prefixes = prefixes,
 def_prefix = c(openbiodiv = "http://openbiodiv.net/"),
 simple_lookup)
...
`identifier_factory` returns a function with just one argument that wraps
around an `findentifier` call.
...
```
seventies_show = openbiodiv_id(seventies_show_label$squote)

sample_paper = RDF$new()
sample_paper$add_triple(
  subject = seventies_show,
  predicate = rdfs_label,
  object = seventies_show_label)
