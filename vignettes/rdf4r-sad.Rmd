...
2. [fidentifier(label, fun = list(...), prefixes, def_prefix, ...)](#identifier-consturction)
3. [identifier_factory(fun = list(...), prefixes, def_prefix, ...)](#manufacturing-identifier-constructors)

##### Identifier ConsturctionFUN = list(...),
...
unique part and returns the list. If multiple prefixes are supplied,
`identifier` will only use the first one. If multiple id's are supplied 
...
sample_id2 = fidentifier("Teodor Georgiev", prefixes = prefixes, f = simple_lookup)
```

##### Manufacturing Identifier Constructors

`identifier_factory` returns a function with just one argument, `label`,
that wraps around an `findentifier` call.
